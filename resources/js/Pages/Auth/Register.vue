        ...
        <div class="w-full max-w-md relative z-10">
            ...
            <div class="bg-white/90 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/20 shadow-2xl transform transition-all duration-500 hover:shadow-3xl animate-fade-in-up">
                ...
                <div class="px-6 py-6 relative overflow-hidden">
                    ...
                    <h1 class="text-2xl font-bold text-black text-center relative z-10">បង្កើតគណនី</h1>
                </div>
                ...
                <div class="px-6 py-6">
                    ...
                    >
                        <div class="space-y-4">
                            <!-- Name Field -->
                            <vee-field name="name" v-slot="{ field, errors, meta }">
                                <div class="space-y-1.5 group">
                                    <Label for="name" class="text-xs font-bold text-gray-800 tracking-wide flex items-center gap-1.5">
                                    ...
                                    <div class="relative">
                                        ...
                                        />
                                        <!-- Success Icon -->
                                        <div v-if="meta.valid && meta.touched && !errors.length && !form.errors.name" class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none animate-scale-in">
                            ...
                            <vee-field name="password" v-slot="{ field, errors, meta }">
                                <div class="space-y-1.5 group">
                                    ...
                                    <p v-else-if="form.errors.password" class="text-xs text-red-600 flex items-center gap-1.5 ml-1 animate-shake">
                                        <svg class="h-3.5 w-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                        </svg>
                                        <span class="font-medium">{{ form.errors.password }}</span>
                                    </p>
                                </div>
                            </vee-field>

                            <!-- Password Confirmation Field -->
                            <vee-field name="password_confirmation" v-slot="{ field, errors, meta }">
                                <div class="space-y-1.5 group">
                                    <Label for="password_confirmation" class="text-xs font-bold text-gray-800 tracking-wide flex items-center gap-1.5">
                                        <div class="w-1 h-3 bg-gradient-to-b from-indigo-500 to-purple-500 rounded-full"></div>
                                        បញ្ជាក់លេខសម្ងាត់
                                    </Label>
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center gap-1.5 z-10">
                                            <button
                                                ...
                                                class="p-1.5 rounded-lg hover:bg-indigo-50 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
                                            >
                                                <svg v-if="showPasswordConfirmation" class="h-4 w-4 text-gray-500 hover:text-indigo-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                                <svg v-else class="h-4 w-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <p v-else-if="form.errors.password_confirmation" class="text-xs text-red-600 flex items-center gap-1.5 ml-1 animate-shake">
                                        <svg class="h-3.5 w-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                        </svg>
                                        <span class="font-medium">{{ form.errors.password_confirmation }}</span>
                                    </p>
                                </div>
                            </vee-field>
                            ...
                            <div v-if="form.errors && Object.keys(form.errors).length > 0 && !form.errors.name && !form.errors.email && !form.errors.password && !form.errors.password_confirmation"
                    ...
                    <div class="text-center">
                        <p class="text-sm text-gray-600">
                            មានគណនីរួចហើយ?
                            <Link href="/login" class="font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 transition-all underline decoration-2 underline-offset-2 decoration-indigo-400">
                                ចូលគណនីឥឡូវនេះ
                            </Link>
                        </p>
                    </div>
...
const validationSchema = yup.object().shape({
    name: yup.string().required('ឈ្មោះត្រូវបានទាមទារ').min(2, 'ឈ្មោះត្រូវមានយ៉ាងហោចណាស់ 2 តួអក្សរ'),
    ...
    password_confirmation: yup.string()
        ...
        .oneOf([yup.ref('password')], 'លេខសម្ងាត់មិនត្រូវគ្នា')
})
